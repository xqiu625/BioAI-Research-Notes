...
### 1. 批次效应处理（Batch Effects）
- **大数据集处理**：
  - 当处理大型数据集（如scTab，包含249个数据集）时，不需要显式地校正批次效应
  - 大量数据集的整合会自然减少单个批次的相对影响
  
- **小数据集处理**：
  - 使用较少数据集时（如文中的三个肺部数据集），需要加入批次协变量
  - 在模型中添加批次协变量来改善数据整合性能
  - 使用SSL-shallow模型：只微调零样本SSL模型的最后一个编码器层，同时使用批次协变量

### 2. 内存需求管理（Memory Management）
- **硬件要求**：
  - GPU：Tesla V100或同等产品，32GB+ GPU内存
  - 系统内存：160GB用于完整scTab数据集（批量大小8,192）

- **优化策略**：
  - 减小批量大小（batch size）可以降低内存需求
    - 例如：批量大小2可以将内存使用降到1GB以下
    - 但会显著增加训练时间（>200小时/epoch）
  - 在较小数据集上进行训练时可以使用较小的批量大小

### 3. 计算资源优化
- **训练时间管理**：
  - 所有预训练方法使用单个GPU训练2天，带有早停
  - 所有微调方法使用单个GPU训练1天，带有早停
  - 使用早停策略避免过度训练
